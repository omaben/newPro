    ...
    <div class="settings">
        <van-nav-bar
            ...
            <van-grid direction="horizontal" :column-num="3" :gutter="10" class="search">
                ...
                <van-grid-item>
                   <van-button type="danger" @click='withdrawRecord()'>查询</van-button>
                </van-grid-item>
            </van-grid>
            ...
            <van-row class="grid-white" v-if='withdrawRecordList.length>0'>
            <van-cell-group class='recharge-info' v-for="(item,index) in withdrawRecordList" :key="index">
                <van-cell title="下单时间" :value="item.created_at" />
                <van-cell title="下单金额" :value="item.money" />
                <van-cell title="回款状态" v-if="item.status === '1'" value="审核中" />
                <van-cell title="回款状态" v-if="item.status === '2'" value="已完成" />
                <van-cell title="回款状态" v-if="item.status === '3'" value="已拒绝" />
            </van-cell-group>
            </van-row>
...
export default class WithdrawList extends Vue {
    ...
    private withdrawRecordList?: any = [];
  ...
  this.beforedate = new Date();
   ...
   this.beforedate= moment(this.beforedate).format("YYYY-MM-DD")
    ...
    private withdrawRecord() {
        ...
        withdraws(post).then((res) => {
        if (res.code === 200) {
            this.withdrawRecordList = res.data.data
        } else {
             if(res.code === 8888){
                 this.goPage('/login')
            }
