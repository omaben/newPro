    ...
    <div class="settings">
        <van-nav-bar
            ...
            <van-grid direction="horizontal" :column-num="3" :gutter="10" class="search">
                ...
                <van-grid-item>
                   <van-button type="danger" @click='getRechargRecord()'>查询</van-button>
                </van-grid-item>
            </van-grid>
            ...
            <van-row class="grid-white" v-if='rechargRecordList.length>0'>
            <van-cell-group class='recharge-info' v-for="(item,index) in rechargRecordList" :key="index">
                <van-cell title="下单时间" :value="item.created_at" />
                <van-cell title="下单金额" :value="item.money" />
                <van-cell title="回款状态" v-if="item.status === '1'" value="成功" />
                <van-cell title="回款状态" v-if="item.status === '2'" value="失败" />
            </van-cell-group>
            </van-row>
...
export default class WithdrawList extends Vue {
  ...
  this.beforedate = new Date();
   ...
   this.beforedate= moment(this.beforedate).format("YYYY-MM-DD")
    ...
    private getRechargRecord() {
        const post: any = {
            start_date :this.beforedate,
            end_date : this.afterdate
        };
        recharges(post).then((res) => {
        if (res.code === 200) {
            this.rechargRecordList = res.data.data
        } else {
            Toast.fail(res.msg);
        }
        });
